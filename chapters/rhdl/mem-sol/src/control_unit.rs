...
impl Default for ControlUnit {
    fn default() -> Self {
        Self { state: DFF::new(Bits::<U2>::default()) }
    }
}
...
pub fn cu_kernel(_cr: ClockReset, _i: Flags, q: Q) -> (ControlSignals, D) {
    let mut cs = ControlSignals { t1_oe: bits(0), t1_we: bits(0), t2_oe: bits(0), t2_we: bits(0), ma_oe: bits(0), ma_we: bits(0), ram_oe: bits(0), ram_we: bits(0),
    alu_carry: bits(0), alu_oe: bits(0), alu_sel: AluOpcode::Adc, rf_oe: bits(0), rf_sel: Sel::RA, rf_we: bits(1) };
    match q.state.raw() {
        0 => {
            cs.t1_oe = bits(1);
            cs.t2_we = bits(1);
