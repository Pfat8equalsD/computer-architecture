...
use crate::{alu::{Alu, AluInput}, control_unit::{ControlSignals, ControlUnit, Flags}, doc::write_svg, ram::{Ram, RamImput}, register::{Register, RegisterInput}, register_file::{RegisterFile, RegisterFileInput}};
...
impl Default for Top {
    fn default() -> Self {
        Self{ 
            T1: Register { memory: DFF::new(Bits::<U16>::from(5)) },
            T2: Register::default(),
            MA: Register::default(),
...
pub fn top_kernel(_cr: ClockReset, _i: (), _q: Q) -> (Bits<U16>, D) {
    ...
    let ControlSignals {t1_oe, t1_we, t2_oe, t2_we, ma_oe, ma_we, ram_we, ram_oe, alu_carry, alu_sel, alu_oe,
    rf_sel,rf_oe,rf_we} = _q.Cu;
    _d.T1 = RegisterInput {
        oe: t1_oe,
        we: t1_we,
        data_in: _bus
